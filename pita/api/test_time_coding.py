...
def decode(system_string):
    ...
    while i < len(parts):
        ...
        if parts[i] == "PS" :
            # Check if there are enough parts remaining for PS parameters
            if i + 2 >= len(parts):
                raise ValueError("Invalid parameters for Power Sampling: expected 2 integers after 'PS', but string is truncated.")
            ...
            # Check if there are enough parts remaining for SMC parameters
            if i + 3 >= len(parts):
                raise ValueError("Invalid parameters for SMC: expected 3 integers after 'SMC', but string is truncated.")
            ...
            # Check if there are enough parts remaining for BO parameters
            if i + 2 >= len(parts):
                raise ValueError("Invalid parameters for Best Of Sampling: expected 2 integers after 'BO', but string is truncated.")
            # Check if the next 2 parts are valid digits (non-negative integers)
            if not (parts[i+1].isdigit() and parts[i+2].isdigit()):
