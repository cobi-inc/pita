...
# for self-certainty calculation
...
class SequentialMonteCarloSelfCertainty:
    ...
    def generate(self, prompt: str, num_particles: int = None, tokens_per_step: int = None):
        ...
        while total_tokens_generated < max_tokens:
            ...
            for i in range(num_particles):
                context = particles[i]
                tokens, _, logprobs = self.sampler.sample(context, max_new_tokens) # get tokens and their logprobs

                # append generated tokens 
                particles[i] = context + tokens 

                # find the self-certainty for each token 
                lse = logsumexp(logprobs, axis=1, keepdims=True)
                log_q = logprobs - lse  # log probabilities normalized
                mean_log_q = np.mean(log_q, axis=1, keepdims=True)
                sc_per_token = np.log(self.top_k) + mean_log_q  # self-certainty per token

                sequence_sc = sc_per_token.sum()  # total self-certainty for the generated block
                cum_log_weights[i] += sequence_sc  # accumulate from previous block generation
